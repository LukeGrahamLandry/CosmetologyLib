...
public class CosmeticArgumentType extends ResourceLocationArgument {
    ...
    public CosmeticArgumentType(Supplier<DataStore> backingDataStore){
    ...
    public static CosmeticInfo get(CommandContext<CommandSource> p_107002_, String p_107003_, DataStore backingDataStore) throws CommandSyntaxException {
        ...
        if (!options(backingDataStore).contains(resourcelocation)) {
            ...
            return backingDataStore.getInfo(resourcelocation);
        }
    }
    ...
    private static List<ResourceLocation> options(DataStore backingDataStore){
        return backingDataStore.getAll().stream().map((info) -> info.id).collect(Collectors.toList());
    }
